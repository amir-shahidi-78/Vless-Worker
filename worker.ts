...
let userID_Path: string | false;

const SECRET = '*Aa123456*';

const users: Map<string, number> = new Map();
// users.set('b98b3ba3-e0c0-4a28-97d3-c591f7e9e7cf', new Date('2024-01-26'));
// users.set('b98b3ba3-e0c0-4a28-97d3-c591f7e9e7ce', new Date('2024-01-26'));
	...
	async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
		...
		try {
			...
			if (!upgradeHeader || upgradeHeader !== 'websocket') {
				...
				userID_Path = users.has(url.pathname.slice(1)) && url.pathname;
				...
				switch (url.pathname) {
					...
					case `/sub/${userID_Path}`: {
						...
						let vlessConfig = createVLESSSub(request.headers.get('Host'));
			...
			/** @type {Error} */ let e: Error = err;
			return new Response(e.toString());
		}
	},
};

export async function uuid_validator(request: Request) {
...
export async function hashHex_f(str: string | undefined) {
 ...
 */
async function vlessOverWSHandler(request: Request, env: Env) {
	...
	const log = (/** @type {string} */ info: string, /** @type {string | undefined} */ event?: string | undefined) => {
	...
	let remoteSocketWapper: RemoteSocket = {
		value: null,
	};
 ...
 */
function makeReadableWebSocketStream(webSocketServer: WebSocket, earlyDataHeader: string, log: Function) {
 ...
 */
async function processVlessHeader(vlessBuffer: ArrayBuffer, env: Env) {
	...
	const userExpiration = await env.DB.get(slicedBufferString);
	const currentSessionCount = users.get(slicedBufferString) ?? 0;
	const MAX_SESSION_COUNT = 2;

	isValidUser = userExpiration ? new Date(userExpiration) > new Date() : false;

	console.log(`userID: ${slicedBufferString}`);

	if (!isValidUser || currentSessionCount >= MAX_SESSION_COUNT) {
		return {
			hasError: true,
			message: 'invalid user',
		};
	}
	users.set(slicedBufferString, currentSessionCount + 1);
 ...
 */
async function handleUDPOutBound(webSocket: WebSocket, vlessResponseHeader: ArrayBuffer, log: (str: string) => void) {
 ...
 * @returns {string}
 */
function getVLESSConfig(hostName: string | null) {
	...
	let output = [];
	let header = [];
	const clash_link = `https://subconverter.do.xn--b6gac.eu.org/sub?target=clash&url=https://${hostName}/sub${userID_Path}?format=clash&insert=false&emoji=true&list=false&tfo=false&scv=true&fdn=false&sort=false&new_name=true`;
	header.push(
	...
	header.push(
		`\n<iframe src="https://ghbtns.com/github-btn.html?user=USERNAME&repo=REPOSITORY&type=star&count=true&size=large" frameborder="0" scrolling="0" width="170" height="30" title="GitHub"></iframe>\n\n`
			.replace(/USERNAME/g, '3Kmfi6HP')
			.replace(/REPOSITORY/g, 'EDtunnel')
	);
	header.push(
		`<a href="//${hostName}/sub${userID_Path}" target="_blank">VLESS 节点订阅连接</a>\n<a href="clash://install-config?url=${encodeURIComponent(
			clash_link
		)}" target="_blank">Clash 节点订阅连接</a>\n<a href="${clash_link}" target="_blank">Clash 节点订阅连接2</a></p>\n`
	);
	header.push(``);

	// Generate output string for each userID
	users.forEach((_, userID, map) => {
    ...
    <head>
        ...
        <meta property="og:image" content="https://api.qrserver.com/v1/create-qr-code/?size=500x500&data=${encodeURIComponent(
					`vless://${(userID_Path as string).slice(1)}@${hostName}${commonUrlPart}`
				)}" />
...
function createVLESSSub(hostName: string | null) {
